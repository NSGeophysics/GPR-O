function plotCMPhypSemblance(data,trng,vrng,xmax,linenr,renorm)
% plotCMPhypSemblance(data,trng,vrng,xmax,linenr,renorm)
%
% Plot the hyperbola scores for a CMP line for different travel time ranges and velocity
% ranges to see which one fits best
%
% INPUT:
%
...
for i=1:length(vrng)
    for j=1:length(trng)
        scr(i,j)=hyperbScore(data,trng(j),vrng(i),xmax,linenr);
    end
end
